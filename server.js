...
// Helper function to get Podio access token
async function getPodioToken() {
  try {
    ...
    throw new Error(`Podio auth error: ${error.message}`);
  }
}

// Helper function to calculate payroll date
function calculatePayrollDate(classDate) {
  const basePayroll = new Date(2026, 8, 16); // Sept 16, 2026
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    // Filter classes for this trainer and last 30 days
